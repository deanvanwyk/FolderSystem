...
public class Folder extends Item implements Cloneable{
    ...
    private boolean hasSameFolders(Folder destination){

        Object[] currentFolders = this.getFolders().toArray();
        Object[] copiedFolders = destination.getFolders().toArray();

        Folder[] castedCurrentFolders = new Folder[currentFolders.length];
        Folder[] castedCopiedFolders = new Folder[copiedFolders.length];

        boolean equalFolders = true;

        for(int i = 0; i < currentFolders.length; i++){
            castedCurrentFolders[i] = (Folder) currentFolders[i];
        }

        for(int i = 0; i < copiedFolders.length; i++){
            castedCopiedFolders[i] = (Folder) copiedFolders[i];
        }

        HashSet<String> currentFoldersSet = new HashSet<String>();

        for(int i = 0; i < castedCurrentFolders.length; i++){

            String entry = castedCurrentFolders[i].getName();
            currentFoldersSet.add(entry);
        }

        if(castedCopiedFolders.length == currentFoldersSet.size()){

            int i = 0;
            while(i < castedCopiedFolders.length && equalFolders){

                if(!currentFoldersSet.contains(castedCopiedFolders[i].getName())){
                    equalFolders = false;
                }
                i++;
            }
    ...
    private boolean hasSameFiles(Folder destination){

        Object[] currentFiles = this.getFiles().toArray();
        Object[] copiedFiles = destination.getFiles().toArray();

        File[] castedCurrentFiles = new File[currentFiles.length];
        File[] castedCopiedFiles = new File[copiedFiles.length];

        boolean equalFiles = true;

        for(int i = 0; i < currentFiles.length; i++){
            castedCurrentFiles[i] = (File) currentFiles[i];
        }

        for(int i = 0; i < copiedFiles.length; i++){
            castedCopiedFiles[i] = (File) copiedFiles[i];
        }

        HashSet<String> currentFileSet = new HashSet<String>();

        for(int i = 0; i < castedCurrentFiles.length; i++){

            String entry = castedCurrentFiles[i].getName();
            currentFileSet.add(entry);
        }

        if(castedCopiedFiles.length == currentFileSet.size()){

            int i = 0;
            while(i < castedCopiedFiles.length && equalFiles){

                if(!currentFileSet.contains(castedCopiedFiles[i].getName())){
                    equalFiles = false;
                }
                i++;
            }
